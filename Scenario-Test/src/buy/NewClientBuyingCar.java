...
class NewClientBuyingCar {
	...
	@Test
	/* Same client forgot he has an account and try to create a new one with the same email
	 *  but doesn't work and false should be returned */		
	...
	void CreateClientThenBuyFailing() throws MalformedURLException, RemoteException, NotBoundException, ServiceException {
		...
		if (s instanceof ICars) {
			...
			if (corp instanceof IEmployees) {
				...
				worked = bankservice.addAccount("USD", "hugo7@leonidas23.god", "bestintheworld", "Hugo", "Douangmala",
						"In the Sky", "0606060606", 0);
				worked = carService.buyCar(3, "hugo7@leonidas23.god", "bestintheworld");
	...
	void CreateClientThenBuy() throws MalformedURLException, RemoteException, NotBoundException, ServiceException {
		...
		if (s instanceof ICars) {
			...
			if (corp instanceof IEmployees) {
				...
				System.out.println(carService.buyCar(4, "hugo7@leonidas23.god", "bestintheworld"));
				System.out.println(bankservice.getAccounts()[0].getBalance());
